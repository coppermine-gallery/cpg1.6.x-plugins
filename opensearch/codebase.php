...
/*************************
  ...
  ********************************************
  ...
  $HeadURL: https://svn.code.sf.net/p/coppermine/code/trunk/cpg1.6.x/plugins/opensearch/codebase.php $
  $Revision: 8825M $
**********************************************/
...
function opensearch_meta(){
    ...
    global $CONFIG, $lang_plugin_opensearch;

    return '<link rel="search" href="plugins/opensearch/osd.xml" type="application/opensearchdescription+xml" title="' . sprintf($lang_plugin_opensearch['search'], $CONFIG['gallery_name']).'" />';
}
...
function opensearch_uninstall() {
    ...
    $handle = fopen($osd, "w+");
    if (!$handle) {
        echo '<p style="color: red; text-align: center">'.sprintf($lang_plugin_opensearch['failed_to_open_file'], $osd).'.</p>';
        return 1;
    }
    $content = <<< EOT
...
EOT;
    ...
    if (!$written) {
        echo '<p style="color: red; text-align: center">'.sprintf($lang_plugin_opensearch['failed_to_write_file'], $osd).'</p>';
        fclose($handle);
        return 1;
    }
...
function opensearch_configure() {
    ...
    global $CONFIG, $lang_plugin_opensearch, $lang_common;
    $superCage = Inspekt::makeSuperCage();
    $action = $superCage->server->getEscaped('REQUEST_URI');

    $nameLimit = sprintf($lang_plugin_opensearch['character_limit'], '16');
    $descriptionLimit = sprintf($lang_plugin_opensearch['character_limit'], '1024');
    echo <<< EOT
    <form name="cpgform" id="cpgform" action="{$action}" method="post">
            <table border="0" cellspacing="0" cellpadding="0" width="100%">
              <tr>
                <td class="tableh2" colspan="2">
                  <h3>{$lang_plugin_opensearch['form_header']}</h3>
                </td>
              </tr>
              <tr>
                <td class="tableb" align="right">
                  {$lang_plugin_opensearch['gallery_url']}:
                </td>
                <td class="tableb">
              ...
              <tr>
                <td class="tableb tableb_alternate" align="right">
                  {$lang_plugin_opensearch['display_name']}:
                </td>
              ...
              <tr>
                <td class="tableb" align="right">
                  {$lang_plugin_opensearch['description']}:
                </td>
                <td class="tableb">
