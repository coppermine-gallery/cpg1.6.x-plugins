...
/*************************
  ...
  ********************************************
  ...
  $HeadURL: https://svn.code.sf.net/p/coppermine/code/trunk/cpg1.6.x/plugins/onlinestats/index.php $
  $Revision: 8634M $
**********************************************/
...
if ($superCage->post->keyExists('submit')) {
    ...
    if ($superCage->post->keyExists('duration') && $superCage->post->getInt('duration') >= '0' && $superCage->post->getInt('duration') <= '1440') {
        if ($superCage->post->getInt('duration') != $CONFIG['mod_updates_duration']) {
            ...
            $changes_message .= sprintf($lang_admin_php['config_setting_ok'], $lang_plugin_onlinestats['config_text']) . '<br />';
        }
    }
...
pageheader($lang_pluginmgr_php['configure_plugin'] . ': ' . $lang_plugin_onlinestats['name']);
...
if (in_array('onlinestats', $contentOfTheMainpage_array) != TRUE){
    ...
    cpg_db_query("UPDATE {$CONFIG['TABLE_CONFIG']} SET value='{$CONFIG['main_page_layout']}' WHERE name='main_page_layout'");
    msg_box('', $lang_plugin_onlinestats['config_extra'], '', '', 'warning');
}
if ($changes_message != '') {
    msg_box('', $changes_message, '', '', 'success');
}
starttable('100%',$icon_array['config'] . $lang_pluginmgr_php['configure_plugin'] . ': ' . $lang_plugin_onlinestats['name']);
echo <<<EOT
    <tr>
        <td>
EOT;
