...
/*************************
  ...
  ********************************************
  ...
  $HeadURL: https://svn.code.sf.net/p/coppermine/code/trunk/cpg1.6.x/plugins/usergal_alphatabs/codebase.php $
  $Revision: 8825M $

  Prepared for CPG 1.6 by ron4mac, 2016-04-28
**********************************************/
...
if ($superCage->get->keyExists('cat') && $superCage->get->getInt('cat') == USER_GAL_CAT){
    ...
    function makejumpbox(){
        global $lang_plugin_usergal_alphatabs;
        starttable('100%', $lang_plugin_usergal_alphatabs['jump_by_username'], 26);
        echo '<tr>';
    ...
    function list_users_query(&$user_count)
    {
        ...
        $user_count = cpg_db_num_rows($result, true);
